...
from src.utilities import get_attentions, split_heads, combine_heads, generate_mask, decompose_card_ids
from misc.consts import PAD_TOKEN
...
class BaselineTransformer(nn.Module):
    def __init__(self, vocab_size: int, embedding_size: int, n_attention_heads: int, 
                n_blocks: int, max_sequence_length: int, ff_dims:int, dropout_prob: float, 
                device: str | T.device = "cpu") -> None:
        super().__init__()
    ...
    def forward(self, sequence):
        ...
        # --- Sequence Embedding ---
        embedded = self.token_embedding(sequence)
        
        x = self.dropout(self.pe_block(embedded))

        # --- Transformer stack ---
        mask = generate_mask(sequence, PAD_TOKEN).to(self.device)
